...
RUN_RESPONSE=$(curl -s --location --request POST "https://api.perfai.ai/api/v1/design-analysis-service/apps/schedule-run?app_id=$APP_ID" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ACCESS_TOKEN")

echo " "
echo "==== Design Issue Response ===="
echo "$RUN_RESPONSE" | jq '.'
#DESIGN_RESPONSE=$(echo "$RUN_RESPONSE" | jq -r '.das')
echo " "
    ...
    while [[ "$STATUS" == "PROCESSING" ]]; do
      ...
      # Handle empty or null STATUS_RESPONSE
       ...
       # echo $STATUS_RESPONSE
        ...
        DESIGN_ISSUE=$(echo "$STATUS_RESPONSE" | jq -r '.das')
        
        # Set STATUS to "PROCESSING" if PRIVACY status is null or empty
        STATUS=$(echo "$DESIGN_ISSUE" | jq -r '.status')
        ...
        if  [ "$STATUS" == "COMPLETED"  ]; then
            ...
            NEW_ISSUES=$(echo "$STATUS_RESPONSE" | jq -r '.das.newIssues[]')
            NEW_ISSUES_DETECTED=$(echo "$STATUS_RESPONSE" | jq -r '.das.newIssuesDetected')
            ...
            if [ -z "$NEW_ISSUES" ] ||  [ "$NEW_ISSUES" == null ]; then
              ...
              echo "Complete Design Issue Status: $DESIGN_ISSUE"
            exit 1
         fi
    fi 
